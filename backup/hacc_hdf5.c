...
void hacc_hdf5_with_compound_type(hid_t file_id, int mpi_rank, int mpi_size) {
...
void hacc_hdf5_with_seperate_dataset(hid_t file_id, int mpi_rank, int mpi_size, bool multi) {
    hid_t dset_id;		        /* Dataset ID */
    hid_t dcpl_id;
...
hid_t create_hdf5_file(char* filename) {
    ...
    if(align)
        H5Pset_alignment(fapl_id, 0, atoi(align)*MB);
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "-i") == 0)
        hacc_hdf5_with_seperate_dataset(file_id, mpi_rank, mpi_size, false);
    else if (strcmp(argv[1], "-m") == 0)
        hacc_hdf5_with_seperate_dataset(file_id, mpi_rank, mpi_size, true);
    else
        hacc_hdf5_with_compound_type(file_id, mpi_rank, mpi_size);
